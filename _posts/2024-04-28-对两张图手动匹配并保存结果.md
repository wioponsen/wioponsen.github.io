...
{% raw %}
ref:https://blog.csdn.net/watermelongogogo/article/details/126999202

参考上述文章进行修改，增加结果保存，代码如下：
...
def getHomography():
    loop = True
    while loop:
        root_path1 = filedialog.askopenfilename(title='请选择源图片')
        root_path2 = filedialog.askopenfilename(title='请选择目标图')

        if root_path1 == '' or root_path2 == '':
        ...
        cv2.namedWindow("warp blend", flags=cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
        cv2.resizeWindow("warp blend", W, H)
        cv2.imshow("warp blend", imgRes)
        keyNum = cv2.waitKey(0)
        if keyNum == ord('c') or keyNum == ord('C'):
            # continue
            loop = True
        elif keyNum == ord('s') or keyNum == ord('S'):
            # save and exit
            name_sec = os.path.basename(root_path1).rsplit('.', 1)[0]
            cv2.imwrite(name_sec + '_warp.png', imgOut)
            cv2.imwrite(name_sec + '_blend.png', imgRes)
            np.savetxt(name_sec + '_homo.txt', Homo)
            loop = False
        else:
            # exit
            loop = False
