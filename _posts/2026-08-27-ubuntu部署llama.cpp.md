...
--parallel N（或 -np N）= 最多同时处理 N 路请求 ，用 slot（槽位）管理并发，每个正在进行的对话占用 1 个 slot，总上下文 -c 会在多个 slot 之间分配
...
--top-p0.80
```
Temperature / 温度，控制模型输出的随机性与创造力。范围通常在 0.0 到 2.0 之间, 温度越低（接近 0），模型就越死板、理智、果断。它在选择下一个字时，永远只挑概率最高的那一个“绝对标准答案”；温度越高，模型就越喜欢冒险，选一些奇奇怪怪、概率较低的词（更有创意，但也容易胡说八道）

Top-K 采样, 限制模型在预测下一个字时，只能从概率最高的前 K 个词中进行选择，极大地缩小了硬件的搜索和计算范围.

--top-p， 限制模型只在累计概率达到 P 的前几个最靠谱的词中进行选择。取值范围在 0.0 到 1.0 之间

相当于给模型套上了一个“不准废话、不准瞎想、直接给最标准答案”的紧箍咒。这是纯本地部署时，在不牺牲模型智商的前提下，榨干输出速度最有效的参数组合。
